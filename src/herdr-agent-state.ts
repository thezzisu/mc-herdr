...
function releaseAgentSync() {
  ...
  try {
    ...
    const worker = new Worker(
      `const{createConnection}=require("net");const{workerData}=require("worker_threads");const{path,payload,sab}=workerData;const sig=new Int32Array(sab);const s=createConnection(path);s.on("connect",()=>s.end(payload+"\\n"));s.on("close",()=>{Atomics.store(sig,0,1);Atomics.notify(sig,0)});s.on("error",()=>{Atomics.store(sig,0,1);Atomics.notify(sig,0)});s.setTimeout(400,()=>s.destroy())`,
